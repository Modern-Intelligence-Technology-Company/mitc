...
- **ADR 0003** + **`suite/`**: **single city-server image** (`suite/Dockerfile`) — supervisord + **MediaMTX**, **digital twin** RTSP mesh (`MITC_TWIN_COUNT`), **FastAPI** REST/UI, **SQLite** + thumbnails, stub ALPR; **`suite/docker-compose.yml`** wrapper; spec **§4.7** cross-link; README/MEMORY updates.
